...
bam_files=(10.PSMC/cockermouth/*_marked_sorted.bam)
bam=${bam_files[$PBS_ARRAY_INDEX - 1]}
sample=$(basename "$bam" _marked_sorted.bam) 

for n in {1..10}; do
	psmc -b -p 4+25*2+4+6 10.PSMC/cockermouth/${sample}.split.psmcfa -o 10.PSMC/cockermouth/bootstrap/${sample}.round-${n}.psmc  
done
