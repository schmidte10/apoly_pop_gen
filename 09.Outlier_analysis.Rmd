...
```bash 
plink --vcf data/gatk.filtered.relaxed_studywide.bicolor.vcf.gz --recode --out data/gatk.filtered.relaxed_studywide.bicolor.plink
```

Remove first two columns 
```bash 
cut -f 3- data/gatk.filtered.relaxed_studywide.bicolor.plink.ped > x.delete
```

```bash
paste data/apoly_populations_metadata_POPIND.txt x.delete > data/gatk.filtered.relaxed_studywide.bicolor.plink.ped
```
```bash
tr -d '\r' < data/gatk.filtered.relaxed_studywide.bicolor.plink.ped > data/gatk.filtered.relaxed_studywide.bicolor.plink.ped
```

```bash
plink --file data/gatk.filtered.relaxed_studywide.bicolor.plink --allow-extra-chr --freq counts --family --out data/apoly_populations
``` 

```bash
npop2=12 #the number of populations you have times 2 
tail -n +2 data/apoly_populations.frq.strat | awk '{ $9 = $8 - $7 } 1' | awk '{print $7,$9}' | tr " " "\n" | awk -v pp=${npop2} '{if (NR % pp == 0){a=a $0"";print a; a=""} else a=a $0" "}' > data/apoly_populations_baypass.txt 
```
## run BAYPASS 

```bash 
g_baypass singularity run /fast/tmp/containers/baypass_public-v2.41.sif g_baypass -npop 6 -gfile data/apoly_populations_baypass.txt -outprefix apoly_populations_baypass -nthreads 4
